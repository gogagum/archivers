...
int main(int argc, char* argv[]) {
  try {
    ...
    const auto ctxCellsCnt =
        takeWithLog("Context cells count: ", std::uint8_t{});
    const auto ctxCellLength =
        takeWithLog("Context cell bit length: ", std::uint8_t{});
    const auto wordsCount = takeWithLog("Words count: ", std::uint64_t{});
    const auto bitsCount = takeWithLog("Bits count: ", std::uint64_t{});

    auto dict = ael::dict::AdaptiveDContextualDictionary(
        {symBitLen, ctxCellsCnt, ctxCellLength});

    DecodeImpl::process(cfg.decoded, dict, wordsCount, bitsCount, symBitLen,
                        tailSize, cfg.fileOpener.getOutFileStream(),
                        cfg.outStream);
