#ifndef BINARY_FIND_HPP
#define BINARY_FIND_HPP
...
class binary_find_kernel : public boost::compute::detail::meta_kernel
{
public:
        ...
        : meta_kernel("binary_find")
    {
        ...
        m_index_arg = add_arg<boost::compute::uint_ *>(boost::compute::memory_object::global_memory, "index");
        m_block_arg = add_arg<boost::compute::uint_>("block");

        boost::compute::atomic_min<boost::compute::uint_> atomic_min_uint;

        *this <<
            "uint i = get_global_id(0) * block;\n" <<
            decl<value_type>("value") << "=" << var<boost::compute::uint_>("i") << ";\n" <<
            "if(" << predicate(var<value_type>("value")) << ") {\n" <<
                atomic_min_uint(var<boost::compute::uint_ *>("index"), var<boost::compute::uint_>("i")) << ";\n" <<
            "}\n";
    }
...
{
    const boost::compute::device &device = queue.get_device();

    boost::shared_ptr<boost::compute::detail::parameter_cache> parameters =
        boost::compute::detail::parameter_cache::get_global_cache(device);

    const std::string cache_key = "__boost_binary_find";

    size_t find_if_limit = 128;
    size_t threads = parameters->get(cache_key, "tpb", 128);
    size_t count = boost::compute::detail::iterator_range_size(first, last);

    InputIterator search_first = first;
    InputIterator search_last = last;

    boost::compute::detail::scalar<boost::compute::uint_> index(queue.get_context());
    ...
    while(count > find_if_limit) {
        index.write(static_cast<boost::compute::uint_>(count), queue);

        // set block and run binary_find kernel
        boost::compute::uint_ block = static_cast<boost::compute::uint_>((count - 1)/(threads - 1));
        ...
        count = boost::compute::detail::iterator_range_size(search_first, search_last);
    }

    return std::find_if(search_first, search_last, predicate);
}

} // end boost namespace


#endif // BINARY_FIND_HPP
