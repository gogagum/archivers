...
TEST(BytesWordFlow, Construct) {
    ...
    [[maybe_unused]] auto wf = BytesWordFlow<BytesWord<2>>(data);
}

//----------------------------------------------------------------------------//
TEST(BytesWordFlow, TailSize0) {
    auto data = std::array<std::byte, 15>{};
    auto wf = BytesWordFlow<BytesWord<2>>(data);
...
TEST(BytesWordFlow, TailSize1) {
    auto data = std::array<std::byte, 15>{};
    auto wf = BytesWordFlow<BytesWord<3>>(data);
...
TEST(BytesWordFlow, TailSize2) {
    ...
    auto wf = BytesWordFlow<BytesWord<4>>(data);
