...
TEST(BytesWordFlow, Construct) {
    ...
    [[maybe_unused]] auto wf =
            ga::fl::BytesWordFlow<ga::w::BytesSymbol<2>>(data);
}

//----------------------------------------------------------------------------//
TEST(BytesWordFlow, TailSize0) {
    auto data = std::array<std::byte, 15>{};
    auto wf = ga::fl::BytesWordFlow<ga::w::BytesSymbol<2>>(data);
...
TEST(BytesWordFlow, TailSize1) {
    auto data = std::array<std::byte, 15>{};
    auto wf = ga::fl::BytesWordFlow<ga::w::BytesSymbol<3>>(data);
...
TEST(BytesWordFlow, TailSize2) {
    ...
    auto wf = ga::fl::BytesWordFlow<ga::w::BytesSymbol<4>>(data);
